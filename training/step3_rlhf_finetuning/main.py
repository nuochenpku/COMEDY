...
from utils.data.data_utils import create_prompt_dataset, MiniDataset, DataCollatorRLHF, get_unsupervised_data, get_prompt_data
...
def parse_args():
    ...
    parser.add_argument('--max_seq_len', type=int, default=0) # stage3 不要用这个，用prompt+ans那个长度！！！
...
def create_datasets(args, tokenizer, train_phase=3):
    ...
    if unsupervised_training_enabled:
        unsupervised_train_dataset = get_unsupervised_data(args, tokenizer, args.unsupervised_dataset_name, train_phase)
    else:
...
def main():
    ...
    print_rank_0(f"len of unsupervised_train_data_loader is {len(unsupervised_train_dataloader)}")
        ...
        for step, (batch_prompt, batch_unsupervised) in enumerate(
            ...
            if exp_dataset is not None:
                ...
                if args.global_rank == 0:
                    writer.add_scalar("Train/actor_loss", actor_loss_sum/inner_iter,  global_step)
                    writer.add_scalar("Train/critic_loss", critic_loss_sum/inner_iter,  global_step)
                    writer.add_scalar("Train/unsuper_loss", unsup_loss_sum/inner_iter,  global_step)
                print_rank_0(
                ...
                if torch.distributed.get_rank() == 0:
                    writer.add_scalar("Train/reward_score", average_reward/inner_iter,  global_step)
                print_rank_0(
                    ...
                    if args.actor_zero_stage == 3:
                        ...
                        if args.enable_ema:
                            save_zero_three_model(rlhf_engine.actor_ema,
                                                ...
                                                global_rank=args.global_rank,
                                                save_dir=os.path.join(
                                                    args.output_dir, 'actor_ema'+ '/' + str(global_step)),
                                                zero_stage=args.actor_zero_stage)
                    else:
                        if args.global_rank == 0:
